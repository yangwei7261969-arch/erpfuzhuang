...
export default function ProcessRouteCreatePage() {
  ...
  useEffect(() => {
    ...
    if (copyFromId) {
      ...
      const original = routes.find(r => r.id === copyFromId);
      if (original) {
        setRouteCode(original.routeCode + '-COPY');
        setRouteName(original.routeName + '(副本)');
        setStyleNo(original.styleNo);
        setCategory(original.category);
        setIsActive(original.isActive);
        setProcesses(original.processes.map(p => ({...p, id: Date.now().toString() + p.processCode})));
      }
    }
  }, [copyFromId]);
  ...
  const loadData = () => {
    ...
    const todayRoutes = routes.filter(r => r.routeCode.includes(today));
    setRouteCode(`RT${today}${(todayRoutes.length + 1).toString().padStart(3, '0')}`);
  };
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-2xl font-bold">{copyFromId ? '复制工艺路线' : '新增工艺路线'}</h1>
            <p className="text-muted-foreground">配置产品生产的工序流程</p>
          </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="space-y-2">
                <Label>路线名称 *</Label>
                <Input value={routeName} onChange={(e) => setRouteName(e.target.value)} placeholder="如：T恤标准工艺" />
              </div>
              <div className="space-y-2">
                <Label>适用款号 *</Label>
                <Input value={styleNo} onChange={(e) => setStyleNo(e.target.value)} placeholder="如：TS-2024-001" />
              </div>
              <div className="space-y-2">
                <Label>品类</Label>
